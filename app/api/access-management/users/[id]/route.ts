  ...
  try {
    ...
    if (roleIds !== undefined) {
      ...
      const formattedUser = await prisma.$transaction(async (tx: { 
        ...
        const existingRoleIds = existingUserRoles.map((ur: { roleId: string }) => ur.roleId);

        const rolesToDisconnect = existingRoleIds.filter((roleId: string) => !roleIds.includes(roleId));
        ...
        if (rolesToDisconnect.length > 0) {
          userRoleUpdate.deleteMany = rolesToDisconnect.map((roleId: string) => ({ roleId: roleId, userId: userId }));
        }
